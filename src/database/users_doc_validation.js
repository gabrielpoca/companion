module.exports = function(newDoc, oldDoc, userCtx, secObj) {
  ...
  if (!newDoc.email) {
    throw { forbidden: "doc.email is required" };
  }

  if (!newDoc.hashed_password) {
    throw { forbidden: "doc.hashed_password is required" };
  }
  ...
  if (newDoc.email.indexOf("@") === -1) {
    throw { forbidden: "doc.email is not valid" };
  }
  ...
  if (oldDoc) {
    ...
    if (oldDoc.email !== newDoc.email) {
      throw { forbidden: "Emails can not be changed." };
    }
  }
